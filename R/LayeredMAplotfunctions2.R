#' Oct16,2025 版に対して、以下の改良
#' 　層化変数を変数番号で呼び出せるようにした
#' 　エラーバーを選択表示できるようにした。なし、標本誤差、標準偏差、標準誤差
#' 　plotのlegendを選択できるようにした。ない、右、上、左、下
#' 　
#' 層化MAplotの呼び出し方法
#'
#' @param df 分析対象の含まれたdf
#' @param selected_vars 分析対象のMAの変数名、もしくは変数番号
#' @param layer_val 層化する変数名(char)　<- 変数番号もOKにする2025/10/20
