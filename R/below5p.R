...
below5p <- function(df){
  ...
  below5p_vec　<-　NULL
  for (i in col_labels){
    active %>% count(!!as.name(i)) %>% mutate(rate=round(100*n/sum(n),2)) -> tmp
    print(tmp)
    tmp %>% rename(V1 = 1) -> tmp2
    tmp %>% filter(rate <= 5) %>% select(1) %>% unlist(1) -> below5p
    below5p_vec_add <- str_c(i,".",below5p)
    below5p_vec <-　append(below5p_vec,below5p_vec_add)
    ggplot(tmp2,aes(x=V1,y=rate)) + geom_col(fill="lightblue") + ggtitle(i) -> p
    plot(p)
  }
